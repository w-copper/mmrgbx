...
class MultiImgResizeToMultiple(BaseTransform):
    ...
    def __repr__(self):
        ...
        repr_str += (
            f"(size_divisor={self.size_divisor}, interpolation={self.interpolation})"
        )
        return repr_str
...
class MultiImgCLAHE(BaseTransform):
    ...
    def __repr__(self):
        repr_str = self.__class__.__name__
        repr_str += (
            f"(clip_limit={self.clip_limit}, tile_grid_size={self.tile_grid_size})"
        )
        return repr_str
...
class MultiImgAdjustGamma(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        results["img"] = [
            mmcv.lut_transform(np.array(img, dtype=np.uint8), self.table)
            for img in results["img"]
        ]
...
class MultiImgPhotoMetricDistortion(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        contrast_mode = random.randint(2) if self.consistent_contrast_mode else None
        results["img"] = [
            _photo_metric_distortion(img, contrast_mode) for img in results["img"]
        ]
...
class MultiImgRandomRotFlip(BaseTransform):
    ...
    def random_rot_flip(self, results: dict) -> dict:
        ...
        results["img"] = [np.flip(img, axis=axis).copy() for img in results["img"]]
        for key in results.get("seg_fields", []):
    ...
    def random_rotate(self, results: dict) -> dict:
        ...
        results["img"] = [
            mmcv.imrotate(img, angle=angle, border_value=self.pad_val)
            for img in results["img"]
        ]
        for key in results.get("seg_fields", []):
            results[key] = mmcv.imrotate(
...
class MultiImgResizeShortestEdge(BaseTransform):
    ...
    def __init__(self, scale: Union[int, Tuple[int, int]], max_size: int) -> None:
        ...
        self.resize = TRANSFORMS.build(
            {"type": "MultiImgResize", "scale": 0, "keep_ratio": True}
        )
...
class MultiImgExchangeTime(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        if exchange:
            ...
            if (
                "gt_seg_map_from" in results["seg_fields"]
                and "gt_seg_map_to" in results["seg_fields"]
            ):
                results["gt_seg_map_from"], results["gt_seg_map_to"] = (
                    results["gt_seg_map_to"],
                    results["gt_seg_map_from"],
                )
        return results
    ...
    def _random_scale(self) -> tuple:
        ...
        if is_tuple_of(self.scale, int):
            ...
            raise NotImplementedError(
                f'Do not support sampling function for "{self.scale}"'
            )
...
class MultiImgPad(BaseTransform):
    ...
    def _pad_img(self, results: dict) -> None:
        ...
        padded_imgs = [
            mmcv.impad(img, shape=size, pad_val=pad_val, padding_mode=self.padding_mode)
            for img in results["img"]
        ]
...
@TRANSFORMS.register_module()
class GridWavePS(BaseTransform):
    def __init__(self, amp=[3, 7], freq=[3, 15], randomize=True):
        self.amp = amp
        self.freq = freq
        self.randomize = randomize

    # 定义波浪形变扭曲函数
    def wave_distortion_map(self, W, H, amplitude=10, frequency=0.1):
        # 创建网格坐标
        x = np.arange(W)
        y = np.arange(H)
        ...
        # 归一化 x 坐标用于正弦函数
        x_normalized = x_grid / W

        # 计算新坐标
        new_x = x_normalized * W  # x 不变
        new_y = y_grid + amplitude[0] * np.sin(2 * np.pi * frequency[0] * x_normalized)

        y_normalized = y_grid / H
        new_x = x_grid + amplitude[1] * np.sin(2 * np.pi * frequency[1] * y_normalized)
        # new_y = y_normalized * H  # y 不变

        new_y[new_y < 0] = 0
        new_y[new_y >= H] = H - 1
        new_x[new_x < 0] = 0
        new_x[new_x >= W] = W - 1
        # 转换为 float32 类型以供 cv2.remap 使用
        map_x = new_x.astype(np.float32)
        map_y = new_y.astype(np.float32)

        return map_x, map_y

    def wave(self, img, amplitude1, amplitude2, frequency1, frequency2):
        W, H = img.shape[1], img.shape[0]

        map_x, map_y = self.wave_distortion_map(
            W, H, [amplitude1, amplitude2], [frequency1, frequency2]
        )
    ...
    def transform(self, results: dict) -> dict:
        ...
        for i in range(len(imgs) - 1):
            if self.randomize:
                amplitude1 = np.random.uniform(self.amp[0], self.amp[1])
                amplitude2 = np.random.uniform(self.amp[0], self.amp[1])
                frequency1 = np.random.uniform(self.freq[0], self.freq[1])
                frequency2 = np.random.uniform(self.freq[0], self.freq[1])
            else:
                amplitude1 = self.amp[0]
                amplitude2 = self.amp[1]
                frequency1 = self.freq[0]
                frequency2 = self.freq[1]
...
class MultiImgAlbu(BaseTransform):
    ...
    def mapper(d: dict, keymap: dict) -> dict:
        ...
        if updated_dict.get("img", None) is not None:
            updated_dict["img"] = np.split(
                updated_dict["img"], indices_or_sections=2, axis=-1
            )
        return updated_dict
