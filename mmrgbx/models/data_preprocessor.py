...
def stack_batch(
    ...
    assert inputs[0].ndim == 3, (
        f"Expected tensor dimension to be 3, but got {inputs[0].ndim}"
    )
    ...
    for i in range(len(inputs)):
        ...
        if data_samples is not None:
            ...
            if hasattr(data_sample, "ps_seg_map"):
                ps_seg_map = data_sample.ps_seg_map.data
                del data_sample.ps_seg_map.data
                data_sample.ps_seg_map.data = F.pad(
                    ps_seg_map, padding_size, value=seg_pad_val
                )
...
class MultiInputSegDataPreProcessor(BaseDataPreprocessor):
    ...
    def forward(self, data: dict, training: bool = False) -> Dict[str, Any]:
        ...
        inputs = data["inputs"]  # [ ]
        inputs = [
            _input.view(-1, 3, _input.shape[-2], _input.shape[-1]) for _input in inputs
        ]
        ...
        inputs = [
            _input.view(-1, _input.shape[-2], _input.shape[-1]) for _input in inputs
        ]
        if training:
