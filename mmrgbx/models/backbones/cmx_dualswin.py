...
class DualSwinTransformer(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__()
        assert arch in self.ARCH
        setting = self.ARCH[arch]
        setting.update(kwargs)

        self.num_layers = len(setting["depths"])
        self.pretrain_img_size = img_size
        self.embed_dim = setting["embed_dim"]
        self.ape = setting["ape"]
        self.patch_norm = setting["patch_norm"]
        ...
        self.pos_drop = nn.Dropout(p=setting["drop_rate"])
        self.pos_drop_d = nn.Dropout(p=setting["drop_rate"])
        depths = setting["depths"]
        num_heads = setting["num_heads"]
        window_size = setting["window_size"]
        mlp_ratio = setting["mlp_ratio"]
        qkv_bias = setting["qkv_bias"]
        qk_scale = setting["qk_scale"]
        drop_rate = setting["drop_rate"]
        attn_drop_rate = setting["attn_drop_rate"]
        drop_path_rate = setting["drop_path_rate"]
        # norm_fuse = nn.LayerNorm
        # stochastic depth
        dpr = [
            x.item() for x in torch.linspace(0, drop_path_rate, sum(setting["depths"]))
        ...
        for i_layer in range(self.num_layers):
            ...
            fr = FeatureRectifyModule(dim=int(self.embed_dim * 2**i_layer), reduction=1)
            self.FRMs.append(fr)
            # patch merging layer
            if i_layer < self.num_layers - 1:
                downsample = PatchMerging(
                    dim=int(self.embed_dim * 2**i_layer), norm_layer=norm_layer
                )
