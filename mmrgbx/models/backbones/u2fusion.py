...
def sequential(*args):
    if len(args) == 1:
        if isinstance(args[0], dict):
            raise NotImplementedError(
                "[ERROR].sequential() does not support OrderedDict"
            )
        else:
            return args[0]
    modules = []
...
class DenseLayer(nn.Module):
    def __init__(self, num_channels, growth):
        ...
        self.conv = ConvBlock(
            num_channels, growth, kernel_size=3, act_type="lrelu", norm_type=None
        )
...
class U2DenseNet(nn.Module):
    def __init__(
        ...
        super(U2DenseNet, self).__init__()
        ...
        self.sub = nn.Sequential(
            ConvBlock(
                self.num_features, 128, kernel_size=3, act_type="lrelu", norm_type=None
            ),
