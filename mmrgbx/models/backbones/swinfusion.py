...
class Cross_SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, y, x_size):
        ...
        if self.shift_size > 0:
            ...
            y = torch.roll(
                shifted_y, shifts=(self.shift_size, self.shift_size), dims=(1, 2)
            )
        else:
            x = shifted_x
            y = shifted_y
...
class CRSTB(nn.Module):
    ...
    def __init__(
        ...
        self.residual_group = Cross_BasicLayer(
            dim=dim,
            input_resolution=input_resolution,
            depth=depth,
            num_heads=num_heads,
        ...
        self.residual_group_A = BasicLayer(
            dim=dim,
            input_resolution=input_resolution,
            depth=depth,
            num_heads=num_heads,
        ...
        self.residual_group_B = BasicLayer(
            dim=dim,
            input_resolution=input_resolution,
            depth=depth,
            num_heads=num_heads,
...
class SwinFusion(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # stochastic depth
        dpr_Ex = [
            x.item() for x in torch.linspace(0, drop_path_rate, sum(Ex_depths))
        ]  # stochastic depth decay rule
        dpr_Fusion = [
            x.item() for x in torch.linspace(0, drop_path_rate, sum(Fusion_depths))
        ]  # stochastic depth decay rule
        ...
        for i_layer in range(self.Ex_num_layers):
            layer = RSTB(
                ...
                drop_path=dpr_Ex[
                    sum(Ex_depths[:i_layer]) : sum(Ex_depths[: i_layer + 1])
        ...
        for i_layer in range(self.Fusion_num_layers):
            layer = CRSTB(
                ...
                drop=drop_rate,
                attn_drop=attn_drop_rate,
                drop_path=dpr_Fusion[
                    sum(Fusion_depths[:i_layer]) : sum(Fusion_depths[: i_layer + 1])
                ],  # no impact on SR results
                norm_layer=norm_layer,
                downsample=None,
                img_size=img_size,
