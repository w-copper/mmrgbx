...
class FeatureInteraction(nn.Module):
    def __init__(
        self, dim, reduction=1, num_heads=None, sr_ratio=None, norm_layer=nn.LayerNorm
    ):
        super().__init__()
        ...
        self.cross_attn = CrossAttention(
            dim // reduction, num_heads=num_heads, sr_ratio=sr_ratio
        )
...
class FeatureFusion(nn.Module):
    def __init__(
        self, dim, reduction=1, sr_ratio=1, num_heads=None, norm_layer=nn.BatchNorm2d
    ):
        super().__init__()
        self.cross = FeatureInteraction(
            dim=dim, reduction=reduction, num_heads=num_heads, sr_ratio=sr_ratio
        )
...
class FeatureFusion2(nn.Module):
    def __init__(
        ...
        self.channel_emb = ChannelEmbed2(
            in_channels=dim, out_channels=dim, norm_layer=norm_layer
        )
        self.apply(self._init_weights)
