...
class Convlutioanl(nn.Module):
    def __init__(self, in_channel, out_channel):
        ...
        self.conv = nn.Conv2d(
            in_channel, out_channel, kernel_size=7, padding=0, stride=1
        )
...
class DATFuse(nn.Module):
    ...
    def forward(self, input):
        ...
        # print(encode_size)
        if (
            encode_size[0] != self.patches_resolution[0]
            or encode_size[1] != self.patches_resolution[1]
        ):
            encode = F.interpolate(
                encode, size=(self.patches_resolution[0], self.patches_resolution[1])
            )

        Transformer = self.basicLayer(encode, self.patches_resolution)
