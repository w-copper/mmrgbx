...
class SiamEncoderDecoder(BaseSegmentor):
    ...
    def __init__(
        ...
        loss_weights_per_modal: Optional[List[float]] = None,
        ...
        self.loss_weights_per_modal = loss_weights_per_modal
    ...
    def extract_feat(self, inputs: Tensor) -> List[Tensor]:
        ...
        if self.loss_weights_per_modal is not None:
            assert len(self.loss_weights_per_modal) == len(images)
        else:
            self.loss_weights_per_modal = [1.0] * len(images)
        features_per_modals = [list() for _ in range(len(images))]
        for i, img in enumerate(images):
            assert img.shape[1] == self.backbone_inchannels
            feature = self.backbone(img)
            # if self.auxiliary_per_modals:
            for fp in feature:
                features_per_modals[i].append(fp)

        if self.with_neck:
            x = self.neck(features_per_modals)
        else:
