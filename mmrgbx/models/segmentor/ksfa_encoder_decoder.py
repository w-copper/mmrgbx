...
class KsfaEncoderDecoder(BaseSegmentor):
    def __init__(
        self,
        backbone: ConfigType,
        ...
        auxiliary_per_modals: bool = False,
        ...
        self.auxiliary_per_modals = auxiliary_per_modals
    ...
    def extract_feat(
        self,
        inputs: Tensor,
        gt_segmentation: Optional[Tensor] = None,
        ps_segmentations: Optional[Tensor] = None,
    ) -> List[Tensor]:
        """Extract features from images."""
        # `in_channels` is not in the ATTRIBUTE for some backbone CLASS.
        images = torch.split(inputs, self.backbone_inchannels, dim=1)

        outs = self.backbone(images, gt_segmentation, ps_segmentations)
    ...
    def _stack_batch_ps(self, batch_data_samples: SampleList) -> Tensor:
        if hasattr(batch_data_samples[0], "ps_seg_map"):
            gt_semantic_segs = [
                data_sample.ps_seg_map.data for data_sample in batch_data_samples
            ]
            return torch.stack(gt_semantic_segs, dim=0)
        else:
            return None
    ...
    def loss(self, inputs: Tensor, data_samples: SampleList) -> dict:
        ...
        x, score_loss = self.extract_feat(
            inputs,
            self._stack_batch_gt(data_samples),
            self._stack_batch_ps(data_samples),
        )

        losses = dict()
        losses.update({"score_loss": score_loss * self.score_weight})
